  ...
  :mCamera(cam), mGLWindow(glw), mpMap( &map ), mapViewer(&mapviewer), driver(_driver)
{
  ...
  GUI.RegisterCommand("add_marker", GUICommandCallBack, this);
  GUI.RegisterCommand("add_marker2", GUICommandCallBack, this);
  ...
  offset_y = 0;
  thresh = 100;
  patt_name = "Data/patt.kanji";
  patt_name2 = "Data/patt.hiro";
  patt_width = 80.0;
  patt_center[0] =  0.0;
  patt_center[1] =  0.0;
  markerCount = 1;
...
void ARDriver::Init()
{
  ...
  if ((patt_id = arLoadPatt(patt_name)) < 0) {
  	...
  	bgra.create(480,640,CV_8UC4);
  	abgr.create(480,640,CV_8UC4);
...
void ARDriver::Render(CVD::Image<CVD::Rgb<CVD::byte> > inputVideo, SE3<> se3CfromW,  bool bLost)
{

	//Perform Opencv Pixel format conversion
	/*cv::cvtColor(inputVideo,bgra,CV_BGR2RGB,3);
	int from_to[] = { 0,1,  1,2,  2,3,  3,0 };
	cv::mixChannels(&bgra,1,&abgr,1,from_to,4);*/

	//Show AR Markers
		 //ShowARMarkers(inputVideo);
	...
	mnCounter++;
	 ...
	 glMatrixMode(GL_PROJECTION);
	 glLoadIdentity();
	 glMultMatrix(mCamera.MakeUFBLinearFrustumMatrix(0.005, 5000));
	 //glMultMatrix(se3CfromW);
...
//---------------------------------------------------------------------
	 if(!bLost)
	 {
		 position p = {"N", offset_x, offset_y, offset_z};
		 tempMarkerCount = markerCount;
		 if(1 == markerCount)
		 {
			 p.type = "N";
			 markerPositions[tempMarkerCount] = p;
		 }
		 else
		   {
		 	  if(last_modified_marker == "N")
		 	  {
		 		  p.type = "N";
		 		  markerPositions[markerCount] = p;
		 	  }
		 	  if(last_modified_marker == "G")
		 		{
...
void ARDriver::drawMyMarker(SE3<> pose)
{
	...
	argConvGlpara2(trans,_mat,paras);
	glLoadMatrixd(paras);
	...
	double x = markerPositions[tempMarkerCount].markerPosition[0];
	double y = markerPositions[tempMarkerCount].markerPosition[1];
	double z = markerPositions[tempMarkerCount].markerPosition[2];

	while (tempMarkerCount > 0) {
		if (markerCount > tempMarkerCount) {
			x = markerPositions[tempMarkerCount].markerPosition[0] - markerPositions[tempMarkerCount + 1].markerPosition[0];
			y = markerPositions[tempMarkerCount].markerPosition[1] - markerPositions[tempMarkerCount + 1].markerPosition[1];
			z = markerPositions[tempMarkerCount].markerPosition[2] - markerPositions[tempMarkerCount + 1].markerPosition[2];
		}

		double d = 0.0;
		if (markerPositions[tempMarkerCount].type == "G") {
			d = 2.0;
		}

		GLfloat mat_ambient[] = { d, 0.0, 1.0, 1.0 };
		...
		glTranslatef(x,y,z);
		if(markerPositions[tempMarkerCount].type == "N")
...
void ARDriver::argConvGlpara2( TooN::Vector<3> _trans, Matrix<3> rota,double gl_para[16] )
{
    /*int     i, j;
...
{
	driver.CommandList.clear();
	driver.CommandList.assign(Commands.begin(),Commands.end());
...
void ARDriver::GUICommandCallBack(void* ptr, string sCommand, string sParams)
{
 ...
 if(sCommand == "plusx")
 {
	 static_cast<ARDriver*>(ptr)->offset_x += 0.5;
 }
 if(sCommand == "minusx")
  {
	 static_cast<ARDriver*>(ptr)->offset_x -= 0.5;
 ...
 if(sCommand == "plusz")
  {
	 static_cast<ARDriver*>(ptr)->offset_z += 1.0;
  }
 if(sCommand == "minusz")
  {
	 static_cast<ARDriver*>(ptr)->offset_z -= 1.0;
 ...
 if(sCommand == "plusy")
  ...
  if(sCommand == "add_marker")
   {
	  static_cast<ARDriver*>(ptr)->Commands.push_back("N");
	  static_cast<ARDriver*>(ptr)->markerCount  += 1;
	  static_cast<ARDriver*>(ptr)->n_markers += 1;
	  static_cast<ARDriver*>(ptr)->last_modified_marker = "N";
   }
  if(sCommand == "add_marker2")
     {
  	  static_cast<ARDriver*>(ptr)->Commands.push_back("G");
  	  static_cast<ARDriver*>(ptr)->markerCount  += 1;
  	  static_cast<ARDriver*>(ptr)->g_markers += 1;
  	  static_cast<ARDriver*>(ptr)->last_modified_marker = "G";
     }
}

void ARDriver::ShowARMarkers(CVD::Image<CVD::Rgb<CVD::byte> > inputVideo)
{
	ARUint8 *dataPtr;
	...
	//Image<CVD::byte> im(ImageRef(640,480));
	//CVD::glReadPixels(im);

	//CVD::glDrawPixels(imFrame);

	//printf("total image size %d \n",im.data());

	/* grab a vide frame */
	if ((dataPtr = (ARUint8 *)inputVideo.data()) == NULL) {
		printf("data pointer error");
		exit(0);
		arUtilSleep(2);
		return;
	}
	...
	printf("marker num %d : ",marker_num);
	...
	for (j = 0; j < marker_num; j++) {
		...
		if (patt_id2 == marker_info[j].id) {
			if (m == -1) {
				m = j;
			} else if (marker_info[m].cf < marker_info[j].cf) {
				m = j;
			}
		}
	}
	if (k == -1 && m == -1) {
		//argSwapBuffers();
		//printf("k is not set");exit(0);
		return;
	}

	/* get the transformation between the marker and the real camera */
	arGetTransMat(&marker_info[k], patt_center, patt_width, patt_trans);
	arGetTransMat(&marker_info[m], patt_center, patt_width, patt_trans2);
...
void ARDriver::draw( void )
{
    ...
    argConvGlpara(patt_trans2, gl_para2);

    printf(" X %f :: Z %f ",gl_para[12], gl_para[14]);
