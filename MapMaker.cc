			      ...
			      SE3<> &se3TrackerPose)
{
  ...
  error_translation = 0.1 - dTransMagn;
  ...
  //Record  the intial R and t.
    mpMap->Rotation_i = se3TrackerPose.get_rotation();
    mpMap->Translation_i = se3TrackerPose.get_translation();
  ...
  for(unsigned int i=0; i<mpMap->vpKeyFrames.size(); i++)
    {
	  ...
	  mpMap->vpKeyFrames[i]->newPose.get_translation() *= dScale;
    }
  for(unsigned int i=0; i<mpMap->vpPoints.size(); i++)
    {
 ...
 * @param sFixedSet  keframes that are fixed
...
void MapMaker::BundleAdjust(set<KeyFrame*> sAdjustSet, set<KeyFrame*> sFixedSet, set<MapPoint*> sMapPoints, bool bRecent)
{
  ...
  //sstream<<"Starting Bundle Adjust: "<<std::endl;
  ...
  sstream<<"Current Map: "<<mpMap->MapID()<<"--KF: "<<mpMap->vpKeyFrames.size()<<"--Points: "<<mpMap->vpPoints.size();
  sstream<<"--Time duration: "<<asctime(localtime(&t))<<std::endl;

  DebugWriteToFile(sstream.str());
  sstream.clear();*/
}
...
{
  return DistToNearestKeyFrame(kCurrent) > mdWiggleScale * 5.0;
}
...
/*
 Eranda Lakshantha.
 This function was used to capture debug messages into a file.
 */
void MapMaker::DebugWriteToFile(std::string message)
{
	std::ofstream fs(file_name,std::ios_base::app);
	if(!fs)
	{
		cerr<<"Failed to dump file. Exiting...";
		exit(0);
	}
	fs<<message;
	fs.close();
