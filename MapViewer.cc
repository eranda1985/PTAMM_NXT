...
void MapViewer::DrawGrid(double offset_X, double offset_Y, double offset_Z)
{
  ...
  glEnd();
  double x = markerPositions[markerCount].markerPosition[0];
  double y = markerPositions[markerCount].markerPosition[1];
  double z = markerPositions[markerCount].markerPosition[2];
  while(markerCount>0)
  {
	  if(totalCount>markerCount)
	  	  {
	  		 x = markerPositions[markerCount].markerPosition[0] - markerPositions[markerCount+1].markerPosition[0];
	  		mMessageForUser<<"x: "<<markerCount<<" : "<<x<<std::endl;
	  		 y = markerPositions[markerCount].markerPosition[1] -  markerPositions[markerCount+1].markerPosition[1];
	  		 z = markerPositions[markerCount].markerPosition[2] - markerPositions[markerCount+1].markerPosition[2];
	  	  }


	  double d = 0.0;
	  if(markerPositions[markerCount].type == "G")
	  {
		  d = 2.0;
	  }
	  ...
	  glTranslatef(x,y,z);
	  mMessageForUser<<"Marker position dictionary: "<<markerCount<<" : "<<markerPositions[markerCount].markerPosition[0]<<std::endl;
	  if(markerPositions[markerCount].type == "G")
	  	  {
 ...
 */
void MapViewer::DrawMap(SE3<> se3CamFromWorld, double offset_X, double offset_Y, double offset_Z, int count, int n_markercount, int g_markercount, std::string lastmodifiedmarker)
{
  ...
  {
	  p.type = "N";
	  markerPositions[count] = p;
  }
  else
  {
	  if(lastmodifiedmarker == "N")
	  {
		  p.type = "N";
		  markerPositions[count] = p;
	  }
	  if(lastmodifiedmarker == "G")
		{
  ...
  //construct the pathToARMarkerArray
  pathToARMarkerArray.clear();
  while(localCount>0)
  {
	  if(markerPositions[localCount].type == "N")
	  {
		  pathToARMarker[0] = -(currPose.inverse().get_translation()[0]) + markerPositions[localCount].markerPosition[0];
		  pathToARMarker[1] = -(currPose.inverse().get_translation()[2]) + markerPositions[localCount].markerPosition[2];
		  pathToARMarkerArray.push_back(pathToARMarker);
...
bool MapViewer::Is_N_MarkersIncremented(int count)
{
	if ((count - default_NMarkers) == 1) {
		default_NMarkers = count;
	}
	if((totalCount - default_NMarkers) == 0)
	{
		return true;
	}
...
bool MapViewer::Is_G_MarkersIncremented(int count)
{
	if ((count - default_GMarkers) == 1) {
		default_GMarkers = count;
	}
	if((totalCount - (default_GMarkers + 1)) == 0)
	{
		return true;
	}
	return false;
