...
System::System()
  : mGLWindow(ImageRef(1920,1080), "PTAMM")
{
  ...
  mimFrameRGB.resize(ImageRef(640,480));
  ...
  mpTracker = new Tracker(ImageRef(640,480), *mpCamera, mvpMaps, mpMap, *mpMapMaker, this);
  mpMapViewer = new MapViewer(mvpMaps, mpMap, mGLWindow);
  mpARDriver = new ARDriver(*mpCamera, ImageRef(640,480), mGLWindow, *mpMap, *mpMapViewer, ROSDriver);
  ...
  GUI.ParseLine("Menu.AddMenuButton Root \"right\" plusx Root");
  GUI.ParseLine("Menu.AddMenuButton Root \"left\" minusx Root");
  GUI.ParseLine("Menu.AddMenuButton Root \"forward\" plusz Root");
  GUI.ParseLine("Menu.AddMenuButton Root \"backward\" minusz Root");
  GUI.ParseLine("Menu.AddMenuButton Root \"up\" minusy Root");
  GUI.ParseLine("Menu.AddMenuButton Root \"down\" plusy Root");
  ...
  GUI.ParseLine("MapViewerMenu.AddMenuButton Root \"Drive Robot\" drive Root");
  GUI.ParseLine("MapViewerMenu.AddMenuButton Root \"N Marker\" add_marker Root");
  GUI.ParseLine("MapViewerMenu.AddMenuButton Root \"G Marker\" add_marker2 Root");
...
void System::Run()
{
	...
	while(!mbDone)
  {
	...
	mpARDriver->Reset();
      ...
      //mpTracker->TrackFrame2(inputVideo);
      
      if(bDrawMap)
      {
    	mpMapViewer->DrawMap(mpTracker->GetNewPose(),mpARDriver->GetOffSetX(),mpARDriver->GetOffSetY(),mpARDriver->GetOffSetZ(),mpARDriver->markerCount, mpARDriver->n_markers, mpARDriver->g_markers, mpARDriver->last_modified_marker);
      }
      if(bDrawAR)
      {
        if( !mpTracker->IsLost() )
        ...
        mpARDriver->Render(mimFrameRGB, mpTracker->GetNewPose(), mpTracker->IsLost() );
        //mpARDriver->Render(inputVideo, mpTracker->GetNewPose(), mpTracker->IsLost() );
      }
...
void System::NewMap()
{
  ...
  *mgvnLockMap = false;
  ...
  int beforeMap = mpMap->MapID();
  mpMap = new Map();
  mpMap->MapBefore = beforeMap; //Set the previous map
  mpMap->Predecessor = beforeMap; //Set the current map's predecessor
  mvpMaps.back()->Successor = mpMap->MapID(); //Set the old map's successor
  mpMap->mapLockManager.Register( this );
  mvpMaps.push_back( mpMap );
...
void System::BringUp(int argc, char** argv)
{
	glutInit(&argc, argv);
