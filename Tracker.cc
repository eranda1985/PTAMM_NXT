 ...
 * @param mm map maker
 */
Tracker::Tracker(ImageRef irVideoSize, const ATANCamera &c, std::vector<Map*> &maps, Map *m, MapMaker &mm, void* sys) :
  mCurrentKF(c),
  mvpMaps(maps),
  mpMap(m),
  mMapMaker(mm),
...
void Tracker::TrackFrame2(cv::Mat imframe)
{
	inputVideo = imframe;
	//processedVideo.create(inputVideo.rows,inputVideo.cols,CV_64FC3);
	processedVideo = imframe;
	cv::cvtColor(imframe,processedVideo,CV_BGR2GRAY,1);

	//mCurrentKF.Image2 = processedVideo;
	//cvtColor(inputVideo,processedVideo,CV_GRAY2BGR,3);
	BasicImage<byte> bb = BasicImage<byte>(processedVideo.data,ImageRef(640,480));
	Image<byte> xx;
	xx.copy_from(bb);
	TrackFrame(xx,true);
}
...
void Tracker::TrackFrame(Image<CVD::byte> &imFrame, bool bDraw)
{
  ...
  if(mpMap->IsGood())
      ...
      if(mnLostFrames < NUM_LOST_FRAMES)  // .. but only if we're not lost!
	{
			...
			//Check the current status of trail tracking stage
			if (mnInitialStage == TRAIL_TRACKING_COMPLETE) {
				if (mbUserPressedSpacebar) // Hit the first space bar
				{
					mbUserPressedSpacebar = false;
					rotation_m = newPose.inverse().get_rotation();
					translation_m = newPose.inverse().get_translation();
			...
			if (mnInitialStage == TRAIL_TRACKING_STARTED) {
				...
				if (mbUserPressedSpacebar) //Hit the second space bar
				{
					mbUserPressedSpacebar = false;
					//need to record the initial R and t with R' and t'
					...
					ss->NewMap();
					mpMap->IsNewPoseSet = 1;
					SetMiddlePose(rotation_m,translation_m);
	  ...
	  { // Provide some feedback for the user:
		mMessageForUser<<"Translation Error: "<<mMapMaker.error_translation<<std::endl;
...
void Tracker::SetMiddlePose(SO3<> r, Vector<3> t)
{
	if(mpMap->IsNewPoseSet)
	{
		mpMap->Rotation_middle = r;
		mpMap->Translation_middle = t;
	}
}
...
void Tracker::TrackMap()
{
  ...
  //Perform the Linear Transformation
    SO3<> Rot_curr = mse3CamFromWorld.inverse().get_rotation();
    Vector<3> Trans_curr = mse3CamFromWorld.inverse().get_translation();

    Vector<3> Trans_new = (mpMap->Rotation_i*Trans_curr) + mpMap->Translation_i;
    SO3<> Rot_new = mpMap->Rotation_i*Rot_curr;

    if(mpMap->IsNewPoseSet)
    {
    	Trans_new = mpMap->Rotation_middle*Trans_new + mpMap->Translation_middle;
    	Rot_new = mpMap->Rotation_middle*Rot_new;
    	//mMessageForUser<<"Middle pose"<<mpMap->Rotation_middle<<endl;
    }

    SE3<> _new(Rot_new,Trans_new);
    newPose = _new.inverse();
